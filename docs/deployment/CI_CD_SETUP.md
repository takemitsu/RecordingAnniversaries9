...
1. [概要](#概要)
2. [実績データ](#実績データ)
3. [SSH鍵のセキュリティ強化](#ssh鍵のセキュリティ強化)
4. [GitHub Secrets設定](#github-secrets設定)
5. [GitHub Actionsワークフローファイル](#github-actionsワークフローファイル)
6. [手動デプロイ実行方法](#手動デプロイ実行方法)
7. [ロールバック手順](#ロールバック手順)
8. [デプロイ通知（オプション）](#デプロイ通知オプション)
...
| 項目 | 所要時間 | 内容 |
|------|---------|------|
| **CIジョブ** | 1m 30s | Lint/Type Check/Tests（176テスト）/Build |
| **Deployジョブ** | 1m 7s | SSH接続/git pull/npm ci/build/migrate/PM2 restart |
| **総所要時間** | **2m 42s** | CI成功後に自動的にDeployジョブ実行 |

**結果**: 全ステップ成功 ✅

**セットアップ内容**:
- デプロイ専用SSH鍵（ed25519）生成・登録
- GitHub Secrets設定（7つ）
...
**最終更新日**: 2025-11-12（CI/CD初回デプロイ実績追加）
